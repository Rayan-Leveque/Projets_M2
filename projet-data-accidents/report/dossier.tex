...
\item \emph{Définition de l'incohérence} : on ne signale que les cas \textbf{francs}, avec des bornes prudentes pour éviter les faux positifs saisonniers (lever/coucher du soleil très variables). Plein jour déclaré hors de \texttt{[8 h ; 19 h]} → incohérent ; nuit (\texttt{3}/\texttt{4}/\texttt{5}) déclarée dans \texttt{[10 h ; 16 h]} → incohérent. Le crépuscule/aube (\texttt{2 - …}) est \textbf{exclu} du test, car c'est par nature une luminosité d'heure intermédiaire qu'aucune borne fixe ne peut qualifier sans connaître la date exacte. Implémentation à la librairie standard (\texttt{csv.reader}, un seul parcours par année), sans pandas.
\end{itemize}
